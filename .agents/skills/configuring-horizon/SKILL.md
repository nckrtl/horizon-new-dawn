...
Dashboard tab, filter, URL, polling, deferred-prop, and infinite-scroll defects are Inertia client-state concerns, not Horizon worker configuration. Use `inertia-react-development` for the implementation and `testing-horizon-interface` for rendered regression coverage; use `optimizing-horizon-react` when the issue involves overlapping requests or client state correctness.
